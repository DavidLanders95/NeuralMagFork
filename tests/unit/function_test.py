...
def test_from_constant(simple_state):
    f = Function(simple_state).from_constant(2.)
    assert f.avg().cpu() == pytest.approx(2.)

def test_from_constant_with_vector(simple_state):
    f = VectorFunction(simple_state).from_constant([1., 2., 3.])
    assert f.avg().cpu() == pytest.approx([1., 2., 3.])

def test_from_numpy(simple_state):
    f = CellFunction(simple_state).from_numpy(np.arange(simple_state.mesh.num_cells).reshape(simple_state.mesh.n))
