    ...
    if space == "cg":
        for i, j, k in product([0, 1], [0, 1], [0, 1]):
            ...
            if shape == ():
                ...
                for l in range(3):
                    result.append(
                        sp.Symbol(f"_{name}:{space}:{shape}:{[i,j,k,l]}_", real=True)
                        * phi
                        * [N.i, N.j, N.k][l]
                    )
    elif space == "dg":
...
def compile_functional(expr):
    variables = set()
    iexpr = sp.integrate(
        sp.integrate(sp.integrate(expr, (N.x, 0, dx)), (N.y, 0, dy)), (N.z, 0, dz)
    )
    ...
    for symb in iexpr.free_symbols:
        ...
        if space == "cg":
            sidx = ",".join(
                ([[":-1", "1:"][j] if i < 3 else str(j) for i, j in enumerate(idx)])
            )
        else:
...
def linear_form_code(expr):
    ...
    for vsymb in v:
        ...
        if vspace == "cg":
            sidx = ",".join(
                ([[":-1", "1:"][j] if i < 3 else str(j) for i, j in enumerate(vidx)])
            )
        else:
    ...
    for sidx, rhs in cmds.items():
        code += (
            f"    result = result.at[{sidx}].add({rhs}, indices_are_sorted=True,"
            " unique_indices=True)\n"
        )
    code += "    return result\n"
