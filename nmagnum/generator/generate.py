    ...
    if space == "cg":
        for i, j, k in product([0, 1], [0, 1], [0, 1]):
            ...
            if shape == ():
                ...
                for l in range(3):
                    expr += (
                        sp.Symbol(f"_{name}:{space}:{shape}:{[i,j,k,l]}_")
                        * phi
                        * [N.i, N.j, N.k][l]
                    )
    elif space == "dg":
        if shape == ():
            ...
            for l in range(3):
                expr += (
                    sp.Symbol(f"_{name}:{space}:{shape}:{[0,0,0,l]}_")
                    * [N.i, N.j, N.k][l]
                )
    else:
        raise NotImplemented
...
def kernel_cmds(expr):
    ...
    for vsymb in v:
        vexpr = expr.subs([(s, 1.0) if s == vsymb else (s, 0.0) for s in v])
        iexpr = sp.factor(
            sp.integrate(
                sp.integrate(sp.integrate(vexpr, (N.x, 0, dx)), (N.y, 0, dy)),
                (N.z, 0, dz),
            )
        )
        rhs = str(iexpr)
        ...
        for symb in iexpr.free_symbols:
            ...
            condition = None
            if vspace == "cg":
                idx[:3] = [i - j for i, j in zip(idx[:3], vidx[:3])]
                condition = (
                    f"i>={vidx[0]} and i<Nx+{vidx[0]} and j>={vidx[1]} and"
                    f" j<Ny+{vidx[1]} and k>={vidx[2]} and k<Nz+{vidx[2]}"
                )
            sidx = ",".join(
                (
                    [
                        ["i", "j", "k"][i] + ["", "+1", "-1"][j] if i < 3 else str(j)
        ...
        if vshape == ():
            ...
            lhs = f"_result_[{vidx[-1]}]"
        else:
            raise NotImplemented

        if condition not in cmds:
            cmds[condition] = []
        cmds[condition].append(f"{lhs} += {rhs}")
...
def kernel_code(expr):
    ...
    for condition, cmdlist in cmds.items():
