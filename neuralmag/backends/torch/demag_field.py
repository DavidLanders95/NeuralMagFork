...
def h_cell(N_demag, m, material__Ms, rho):
    ...
    if len(dim) == 0:
        hx = torch.zeros_like(m[:, :, :, 0])
        hy = torch.zeros_like(m[:, :, :, 0])
        hz = torch.zeros_like(m[:, :, :, 0])
        for ax in range(3):
            mx = rho * material__Ms * m[:, :, :, ax]
            hx += N_demag[0][ax] * mx
            hy += N_demag[1][ax] * mx
            hz += N_demag[2][ax] * mx
        return torch.stack([hx, hy, hz], dim=3)
...
def h_cell_pbc(m, material__Ms, rho, dx):
    ...
    kx = (
        2.0 * np.pi * torch.arange(n[0], device=m.device, dtype=m.dtype) / n[0]
    ).reshape(-1, 1, 1)
    ky = (
        2.0 * np.pi * torch.arange(n[1], device=m.device, dtype=m.dtype) / n[1]
    ).reshape(1, -1, 1)
    kz = (
        2.0 * np.pi * torch.arange(n[2], device=m.device, dtype=m.dtype) / n[2]
    ).reshape(1, 1, -1)
...
def init_N_component(state, perm, func, p, batch_size=1):
    ...
    ij = [
        torch.fft.fftfreq(s, 1 / s).to(dtype=torch.float64, device=state.device)
        for s in shape
    ]
...
def init_N(state, p, batch_size=1):
    ...
    Nxx = init_N_component(state, [0, 1, 2], demag_f, p, batch_size).to(
        dtype=state.dtype
    )
    Nxy = init_N_component(state, [0, 1, 2], demag_g, p, batch_size).to(
        dtype=state.dtype
    )
    Nxz = init_N_component(state, [0, 2, 1], demag_g, p, batch_size).to(
        dtype=state.dtype
    )
    Nyy = init_N_component(state, [1, 2, 0], demag_f, p, batch_size).to(
        dtype=state.dtype
    )
    Nyz = init_N_component(state, [1, 2, 0], demag_g, p, batch_size).to(
        dtype=state.dtype
    )
    Nzz = init_N_component(state, [2, 0, 1], demag_f, p, batch_size).to(
        dtype=state.dtype
    )
