...
class CodeClass(object):
    def save_and_load_code(self, *args):
        ...
        prefix = "_".join((config.backend.name,) + this_module.parent.parts[-i:] + (this_module.stem,))
        cache_file = f"{prefix}_{hashlib.md5(pickle.dumps(args)).hexdigest()}.py"
        cache_dir = os.getenv("NM_CACHEDIR", pathlib.Path.home() / ".cache" / "neuralmag")
        ...
        if not code_file_path.is_file():
            ...
            logging.info_green(f"[{self.__class__.__name__}] Generate {config.backend.name} core methods")
